...
- **可靠性标记**：按终身样本数判断——`✓` ≥ `Min Samples`（数据足够、门槛可以下判断），`⚠️` ≥ 5 但不足 `Min Samples`（有数字但未经证明），`❌` < 5（无可用数字；仅 `Signal Type`/`Grade` 模式可见，Ranking 会隐藏这类行）。时间衰减不影响该标记——有效样本数存在稳态硬上限（约 `1/(1−0.5^(信号间隔/半衰期))`），若按它判断，稀疏桶将永远卡在 `⚠️`。
...
| ⏳ | 桶样本不足——无结论 | 未经证明，不是被证明差。信号是否仍触发警报取决于 `Unproven Buckets`（默认 `Pass`：触发并标 `⏳`；`Block (Legacy)`：不触发）。Gate 行中 `n=2/20⏳✓`/`⏳✗` 显示样本数及最终判定 |
