...
| **Signal** | 当前信号图标 + 质量等级 + 过滤标记，如 `🔥[A]✓`。标记为三态：`✓` 样本足够且质量门槛通过（验证过、确实好），`⚠️` 样本足够但门槛未过（**验证过、确实差**），`⏳` 样本不足、无法下结论（按 `Unproven Buckets` 放行或拦截）。无新信号时显示持续状态文字，如 `🔥持续`（仍处极端区）或 `超卖区`（处于普通超卖区）。 |
| **Status** | 当前区域：`🟢 EXTREME OVERSOLD`、`🟡 OVERSOLD`、`⚪ NEUTRAL`、`🟠 OVERBOUGHT`、`🔴 EXTREME OVERBOUGHT`。 |
| **Gate** | 仅在启用统计过滤时显示。统计门槛为当前（或潜在）信号实际查询的统计桶及判定结果——即 Signal 行 `✓`/`⏳`/`⚠️` 的"为什么"。`🔥[A]📈 n=2/20⏳✓` = 该桶终身样本只有 2 个、不足要求的 20 个：**数据不够、未经证明——不是被证明差**；末尾标记显示 `Unproven Buckets` 的处理结果（`⏳✓` 绿色 = `Pass` 放行，`⏳✗` 灰色 = `Block (Legacy)` 拦截）。样本足够时每条质量路径显示 `实际→门槛` 及各自的标记，如 `63→67%✗\|+1.1→0.4%✓`（胜率优势路径未过、收益优势路径通过 → `Either Edge` 下门槛整体通过）；门槛通过时单元格为绿色，未过为橙色。`—` = 无可门控对象（中性K线，或纯背离 `↗️`/`↘️` 标记——它们不经过统计门槛）。 |
...
| **Resonance** | 有效数据周期中有多少个方向一致，如 `🟢 3/4`。当所列周期中包含图表自身周期时（如日线图上的 `D`），只计一次——分母变为 3，此时共振需要三个去重后的周期全部一致。末尾的 `⚠️` 表示至少有一个周期没有数据（仅作显示提醒，不改变共振计算）。 |
...
- **`[A]`** —— 质量等级 A–D。
- **`📈`/`📉`** —— 买入桶或卖出桶。
- **`(28)`** —— 终身样本数（不衰减——与 `Min Samples` 门槛、`Gate` 行的 `n` 同口径）。
- **可靠性标记**：按终身样本数判断——`✓` ≥ `Min Samples`（数据足够、门槛可以下判断），`⏳` ≥ 5 但不足 `Min Samples`（有数字但未经证明——新图表上大多数行都会是它，这是诚实，不是警告），`❌` < 5（无可用数字；仅 `Signal Type`/`Grade` 模式可见，Ranking 会隐藏这类行）。全面板词汇表在这里同样成立：`⚠️` 只表示*验证过、确实差*，绝不会出现在这个位置。时间衰减不影响该标记——有效样本数存在稳态硬上限（约 `1/(1−0.5^(信号间隔/半衰期))`），若按它判断，稀疏桶将永远到不了 `✓`。
...
如果排行榜按绝对胜率排，上涨资产里所有买入桶会挤满榜首、所有卖出桶沉底——榜单测的就成了"这只股票在涨"，而不是"哪种信号好用"。（这是 v7.3 的实际缺陷，同一个口径问题也导致卖出警报被系统性过滤掉。）Edge 排序和警报门槛用的是同一把尺：终身样本数 ≥ `Min Samples`（默认 20）时，门槛要求质量优势——胜率优势至少 +5pp（默认），或在 v7.5 起的默认 `Payoff Gate = Either Edge` 下改以收益优势 ≥ `Min Payoff Edge %` 通过；不足 `Min Samples` 时无法下结论，由 `Unproven Buckets` 决定（默认 `Pass`：信号放行并标 `⏳`）。
...
| 标记 | 含义 | 备注 |
|------|------|------|
| ✓ | 样本足够，质量门槛通过 | 验证过、确实好——出现在仪表盘信号行和警报消息中 |
| ⚠️ | 样本足够，质量门槛未过 | **验证过、确实差**——见于 `Alert Only` 或 `Soft` 模式；永不触发警报 |
| ⏳ | 桶样本不足——无结论 | 未经证明，不是被证明差。信号是否仍触发警报取决于 `Unproven Buckets`（默认 `Pass`：触发并标 `⏳`；`Block (Legacy)`：不触发）。Gate 行中 `n=2/20⏳✓`/`⏳✗` 显示样本数及最终判定；统计/排行行中它是可靠性标记的中间档（≥ 5 但不足 `Min Samples`） |
| 🚫 | 信号存在但被隐藏 | 由 Smart 普通信号隐藏、趋势保护或 `Hard` 过滤导致 |
| （无） | 非触发 K 线，或统计过滤已关闭 | 例如 `🔥持续` 这类持续状态文字 |

> 警报只为通过统计过滤的信号触发——警报文本里只会出现 `✓`（验证过）或 `⏳`（未经证明、按默认 `Unproven Buckets = Pass` 放行），永远不会出现 `⚠️`。
...
1. **样本充分性**：终身样本数 ≥ `Min Samples`（默认 20）。桶*不足*这个数时，质量路径无从下结论，改由 **`Unproven Buckets`** 决定：**`Pass`**（默认）放行信号并标 `⏳`——样本不足意味着*未经证明，不是被证明差*；**`Block (Legacy)`** 拦截（原有行为——细粒度桶（`Ranking` 模式 32 个）在新图表上可能数年内拦下所有信号）。`Pass` 下过滤器只做它真正有依据做的事：拦截**被证明**无效的信号。保护随数据积累逐渐生效；想要保守行为就设 `Block (Legacy)`，或把 `Stats Mode` 换成 `Signal Type` 加快样本积累（8 个桶比 32 个快约 4 倍）。
2. 样本足够时由**质量判定**说了算——因此 `⚠️` 永远意味着*验证过、确实差*，绝不是*没数据*。
...
> **还原 v7.4 门槛行为**：设 `Payoff Gate = Off` **且** `Unproven Buckets = Block (Legacy)`。门槛判定即与 v7.4 完全一致（排行榜的收益优势数字和 "No timing edge" 提示只是显示层，在 Edge 模式下仍会出现）。
>
...
- `Tester` —— `All` 列该怎么读
- `Production Gate` —— 当前信号在 `Stats Mode` 下实际选中的统计桶，附终身样本数（不足 `Min Samples` 时尾随 `⏳`）、平均收益、调整胜率，以及（`Edge vs Baseline` 模式下）收缩后收益优势，例如 `Ranking` 模式下的 `EXT[A](12⏳) +2.8%|67%|+1.2%`（`Signal Type` 模式显示 `TYPE:EXT`，`Grade` 模式显示 `GRADE[A]`；无信号时显示 `Idle`；`Absolute (Legacy)` 模式下不带收益后缀）。

指标自带的 `Gate` 行也会出现在策略报告版里（随生产仪表盘继承）。两者的区别：`Gate` 行每根 K 线都跟踪潜在信号（含持续区域状态），显示门槛判定的"实际值→门槛值"；`Production Gate` 只在实际触发 K 线上有内容（其余显示 `Idle`），且选方向时遵循 `Trade Side`。
