...
- **门槛模式**（`Gate Mode`，默认 `Edge vs Baseline`）：胜率门槛相对每个方向的无条件基准胜率衡量——`(最小调整胜率 − 50)` 被解读为要求的超额优势（百分点），贝叶斯先验也收缩到基准胜率；可避免趋势市场中卖出桶被系统性拒绝。有效门槛（`基准 + 优势`）钳制在 25%~90% 之间，防止极端基准下门槛在数学上不可满足或过低；Dashboard 统计标题行按方向显示 `Base→Req`（基准 → 有效门槛）。`Absolute (Legacy)` 保留旧版固定绝对门槛
- **排行榜（Edge 模式）**：`Gate Mode = Edge vs Baseline` 时，`Ranking` 排行榜按每个桶相对自身方向基准的优势（调整胜率 − 方向基准）排序，与门槛逻辑一致，并在数值后追加优势百分点（如 `+4.5%|85%(+23.1pp)`）。有效样本数不足 5 的桶不显示；负优势的桶仍会显示并自然排在最后。`Absolute (Legacy)` 保持按调整胜率的原排序
- 三种过滤模式：
...
- 可选风险退出（均默认关闭）：
  - `Use ATR SL/TP Exits`（`harness_use_risk_exits`）：按警报展示的 ATR 止损/止盈价格退出；价格在信号 K 线收盘时快照（入场单在下一根 K 线开盘成交），`strategy.exit` 与入场单一起下单并通过 `from_entry` 绑定，因此成交当根 K 线即受止损/止盈保护
  - `Max Holding Bars`（`harness_max_holding_bars`，`0` = 关闭）：持仓恰好 N 根 K 线后强制平仓（时间退出）——平仓单在第 N−1 根持仓 K 线收盘时下达，下一根 K 线开盘成交
...
> 两种门槛模式下统计标题行均按方向显示 `Base→Req`（基准 → 有效门槛）——`Edge vs Baseline` 模式下 `Req` 为 `基准 + 优势`（钳制在 25%~90%），`Absolute (Legacy)` 模式下 `Req` 为固定的「最小调整胜率」绝对门槛。排行榜仅在 `Edge vs Baseline` 模式下追加该桶相对自身方向基准的优势百分点，`Absolute (Legacy)` 模式不带 `pp` 后缀。
...
- 统计引擎升级（全部默认**开启**；恢复旧版行为的方法见功能 7 的升级提示）：
  ...
  - 独立采样，避免前瞻收益区间重叠
  - `Edge vs Baseline` 门槛模式，要求相对方向基准胜率的超额优势；有效门槛钳制在 25%~90% 之间并在 Dashboard `Base→Req` 标题行中显示，该模式下 `Ranking` 排行榜按各桶相对自身方向基准的优势排序
...
- 策略壳：可选 ATR 止损/止盈退出和最大持仓 K 线时间退出（均默认关闭）；止损/止盈在信号 K 线收盘时快照，退出单从入场成交当根 K 线起即生效，`Max Holding Bars = N` 实际持仓恰好 N 根 K 线
- 保留 `v7.3` 的正确性修复：
  - `lookback` 不低于统计下限
  - 周线保护使用已确认周线数据
