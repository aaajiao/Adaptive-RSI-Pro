...
| **Gate** | 仅在启用统计过滤时显示。统计门槛为当前（或潜在）信号实际查询的统计桶及判定结果——即 Signal 行 `✓`/`⚠️` 的"为什么"。`🔥[A]📈 n=2/20⏳`（灰色）= 该桶终身样本只有 2 个、不足要求的 20 个：**数据不够、未经证明——不是被证明差**。样本足够时每条质量路径显示 `实际→门槛` 及各自的标记，如 `63→67%✗\|+1.1→0.4%✓`（胜率优势路径未过、收益优势路径通过 → `Either Edge` 下门槛整体通过）；门槛通过时单元格为绿色，未过为橙色。`—` = 无可门控对象（中性K线，或纯背离 `↗️`/`↘️` 标记——它们不经过统计门槛）。 |
...
| ⏳ | Gate 桶样本不足 | 仅 Gate 行——`n=2/20⏳` 表示终身样本 2 个、要求 20 个；信号是未经证明，不是被证明差 |
